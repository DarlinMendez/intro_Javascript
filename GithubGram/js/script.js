let usuarioGitHhub = {
    nombre: "DarlinMendez",
    nombre_usuario: "KinqDarlin",
    url_imagen: "data:image/png;base64,",

};
    


//console.log(usuarioGitHhub);

//JSON.stringify(usuarioGitHhub);

let textoJSON = JSON.stringify(usuarioGitHhub);

//console.log(textoJSON);


let usuarioGitHhubJSON = `
{
    "nombre": "Darlin Mendez"
    "nombre_usuario": "Darlin"
    "url_imagen": "https://google.com"
    "edad": 20
}
`;



//let objetoJSON = JSON.parse(usuarioGitHhubJSON);

/*
console.log(objetoJSON.nombre);
console.log(objetoJSON.nombre_usuario);
console.log(objetoJSON.url_imagen);
console.log(objetoJSON.edad);
*/

let miusuarioJSON = `{
  {
    "login": "DarlinMendez",
    "id": 60099291,
    "node_id": "MDQ6VXNlcjYwMDk5Mjkx",
    "avatar_url": "https://avatars1.githubusercontent.com/u/60099291?v=4",
    "gravatar_id": "",
    "url": "https://api.github.com/users/DarlinMendez",
    "html_url": "https://github.com/DarlinMendez",
    "followers_url": "https://api.github.com/users/DarlinMendez/followers",
    "following_url": "https://api.github.com/users/DarlinMendez/following{/other_user}",
    "gists_url": "https://api.github.com/users/DarlinMendez/gists{/gist_id}",
    "starred_url": "https://api.github.com/users/DarlinMendez/starred{/owner}{/repo}",
    "subscriptions_url": "https://api.github.com/users/DarlinMendez/subscriptions",
    "organizations_url": "https://api.github.com/users/DarlinMendez/orgs",
    "repos_url": "https://api.github.com/users/DarlinMendez/repos",
    "events_url": "https://api.github.com/users/DarlinMendez/events{/privacy}",
    "received_events_url": "https://api.github.com/users/DarlinMendez/received_events",
    "type": "User",
    "site_admin": false,
    "name": null,
    "company": null,
    "blog": "",
    "location": null,
    "email": null,
    "hireable": null,
    "bio": "Aprendiz",
    "public_repos": 3,
    "public_gists": 0,
    "followers": 0,
    "following": 0,
    "created_at": "2020-01-20T12:29:44Z",
    "updated_at": "2020-05-15T05:30:46Z"
  }
}
`;


//let user = JSON.parse(miusuarioJSON);


//seleccionar los elementos

const imgperfil = document.querySelector("img.imagen_perfil")

const titulonombre = document.querySelector("h3.nombre")

const Parrafonombreusuario = document.querySelector("p.nombre_usuario")

const spanseguidores = document.querySelector("span.seguidores")

const spansiguiendo = document.querySelector("span.siguiendo")

const parrafobio = document.querySelector("p.bio")

const btnBuscar = document.querySelector("button#buscar")

const barraBusqueda = document.querySelector("#barra_busqueda");

btnBuscar.addEventListener("click", function(){
  obtenerDatosGithub();
});


obtenerDatosGithub();

//Pasos para obtener los Datos
//1. Crear una fucion asincrona
//2. ejecutar una peticion y esperar por la respuesta
//3. Verificar si la respuesta esta bien
//4. convertir la respuesta y procesarla


//1. Crear una fucion asincrona
async function obtenerDatosGithub(){
  //console.log("vamos a obtenerlos Datos")
  
  let valor = barraBusqueda.value;
  //2. ejecutar una peticion y esperar por la respuesta
  let respuesta = await fetch("https://api.github.com/users/"+valor)

  //3. Verificar si la respuesta esta bien
  if (respuesta.ok) {
    //4. convertir la respuesta y procesarla
    let user = await respuesta.json();

  
    //actualizar perfil
   imgperfil.src = user.avatar_url

   Parrafonombreusuario.textContent = user.login

   spanseguidores.textContent = user.followers
   spansiguiendo.textContent = user.following
    parrafobio.textContent = user.bio
  } 
else{
  console.log("ocurrio un error")
}


}




/*
//actualizar la ruta de la imagen 

imgperfil.src = user.avatar_url;

//actualizar el nomnbre

titulonombre.textContent = user.name;

//actualizar usuario

Parrafonombreusuario.textContent = user.login

//actualizar informacion de seguidores 
spanseguidores.textContent = user.followers
spansiguiendo.textContent = user.following



*/